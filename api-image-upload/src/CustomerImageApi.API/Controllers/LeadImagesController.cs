...
[ApiController]
[Route("api/leads/{leadId}/images")]
[Authorize]
public class LeadImagesController : ControllerBase
{
    private readonly IImageService _imageService;

    public LeadImagesController(IImageService imageService)
    {
        _imageService = imageService;
    }

    [HttpGet]
    [SwaggerOperation(Summary = "Get all images for a lead")]
    [ProducesResponseType(typeof(ApiResponse<ImageListResponse>), 200)]
    [ProducesResponseType(401)]
    public async Task<ActionResult<ApiResponse<ImageListResponse>>> GetLeadImages(int leadId)
    {
        try
        {
            var images = await _imageService.GetLeadImagesAsync(leadId);
            return Ok(new ApiResponse<ImageListResponse>(images, "Lead images retrieved successfully"));
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    [SwaggerOperation(Summary = "Upload a single image for a lead")]
    [ProducesResponseType(typeof(ApiResponse<ImageResponse>), 201)]
    [ProducesResponseType(typeof(ApiResponse<ImageResponse>), 400)]
    [ProducesResponseType(401)]
    public async Task<ActionResult<ApiResponse<ImageResponse>>> UploadLeadImage(int leadId, [FromBody] ImageUploadRequest request)
    {
        try
        {
            var image = await _imageService.UploadLeadImageAsync(leadId, request);
            return CreatedAtAction(nameof(GetLeadImages), new { leadId }, 
    ...
    [HttpPost("bulk")]
    [SwaggerOperation(Summary = "Upload multiple images for a lead (up to the 10-image limit)")]
    ...
    [ProducesResponseType(401)]
    public async Task<ActionResult<ApiResponse<MultipleImageUploadResponse>>> UploadMultipleLeadImages(int leadId, [FromBody] MultipleImageUploadRequest request)
    {
        try
        {
            ...
            var result = await _imageService.UploadMultipleLeadImagesAsync(leadId, request);
    ...
    [HttpDelete("{imageId}")]
    [SwaggerOperation(Summary = "Delete an image from a lead")]
    [ProducesResponseType(typeof(ApiResponse<bool>), 200)]
    [ProducesResponseType(typeof(ApiResponse<bool>), 404)]
    [ProducesResponseType(401)]
    public async Task<ActionResult<ApiResponse<bool>>> DeleteLeadImage(int leadId, int imageId)
    {
        try
        {
            var result = await _imageService.DeleteLeadImageAsync(leadId, imageId);
            if (!result)
                return NotFound(new ApiResponse<bool>($"Image with ID {imageId} not found for lead {leadId}"));
